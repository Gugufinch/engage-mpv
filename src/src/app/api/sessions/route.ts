...
export async function POST(req: NextRequest) {
  try {
    ...
    const { creator_name, topic, context, category } = await req.json();
    ...
    // Only include optional fields if present (avoids errors if columns don't exist yet)
    if (context?.trim()) insertData.context = context.trim();
