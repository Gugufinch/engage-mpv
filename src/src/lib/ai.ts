...
OUTPUT FORMAT:
Return ONLY a valid JSON object (no markdown, no code blocks, no preamble) with exactly these fields:
{
...
Analyze both perspectives and provide your structured recommendation as JSON. Use "${creatorName}" and "${partnerName}" when referencing each person.`;
  ...
  // Also handle if it starts with any non-JSON text before {
